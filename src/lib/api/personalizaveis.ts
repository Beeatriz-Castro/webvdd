...
export async function listPersonalizaveis(
  ...
  if (params.categoriaId) {
    searchParams.append("categoriaId", String(params.categoriaId));
  }
  ...
  if (params.precoMin !== undefined) {
    searchParams.append("precoMin", String(params.precoMin));
  }

  if (params.precoMax !== undefined) {
    searchParams.append("precoMax", String(params.precoMax));
  }
...
export async function listCategorias(): Promise<Categoria[]> {
  const response = await fetch(
    `${API_BASE_URL}/produtos-personalizaveis/categorias`,
  );
  if (!response.ok) {
    throw new Error(`Erro ao listar categorias: ${response.statusText}`);
  }
  return response.json();
}
...
export async function createCategoria(nome: string): Promise<Categoria> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      body?.message || `Erro ao criar categoria: ${response.statusText}`,
    );
  }
  return response.json();
}
...
export async function listTamanhos(): Promise<Tamanho[]> {
  const response = await fetch(
    `${API_BASE_URL}/produtos-personalizaveis/tamanhos`,
  );
  if (!response.ok) {
    throw new Error(`Erro ao listar tamanhos: ${response.statusText}`);
  }
  return response.json();
}
...
export async function listCores(): Promise<Cor[]> {
  const response = await fetch(
    `${API_BASE_URL}/produtos-personalizaveis/cores`,
  );
  if (!response.ok) {
    throw new Error(`Erro ao listar cores: ${response.statusText}`);
  }
  return response.json();
}
...
export async function createCor(nome: string, hexCode: string): Promise<Cor> {
  ...
  if (!response.ok) {
    const body = await response.json().catch(() => null);
    throw new Error(
      body?.message || `Erro ao criar cor: ${response.statusText}`,
    );
  }
  return response.json();
}
...
export async function createPersonalizavel(
  ...
  if (!response.ok) {
    const body = await response.json().catch(() => null);
    throw new Error(
      body?.message || `Erro ao cadastrar produto: ${response.statusText}`,
    );
  }
  return response.json();
}
...
export async function updatePersonalizavel(
  ...
  if (!response.ok) {
    const body = await response.json().catch(() => null);
    throw new Error(
      body?.message || `Erro ao atualizar produto: ${response.statusText}`,
    );
  }
  return response.json();
}
...
export async function deletePersonalizavel(id: number): Promise<void> {
  ...
  if (!response.ok) {
    const body = await response.json().catch(() => null);
    throw new Error(
      body?.message || `Erro ao remover produto: ${response.statusText}`,
    );
  }
}
...
export async function togglePersonalizavelStatus(
  ...
  const response = await fetch(
    `${API_BASE_URL}/produtos-personalizaveis/${id}/status`,
    {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  if (!response.ok) {
    const body = await response.json().catch(() => null);
    throw new Error(
      body?.message || `Erro ao alterar status: ${response.statusText}`,
    );
  }
  return response.json();
}

export const getCustomizableById = async (id: number) => {
  const response = await fetch(
    `${API_BASE_URL}/produtos-personalizaveis/${id}`,
  );
